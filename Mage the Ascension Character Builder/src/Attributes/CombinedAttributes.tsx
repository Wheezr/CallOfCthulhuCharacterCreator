...
function CombinedAttributes() {
  ...
  function change(passed: EventTarget & HTMLSelectElement,
                  list: {value: string, state: boolean, selected: number}[],
                  action: Dispatch<SetStateAction<{value: string, state: boolean, selected: number}[]>>) {
    const selects = list.map((item) => {
      if (passed.value === "-- selection an option --") {
        if (passed.options.selectedIndex === item.selected) {
          ...
          return {value: item.value, state: item.state, selected: item.selected};
        }
      }
      else if (passed.value === item.value) {
        return {value: passed.value, state: true, selected: passed.options.selectedIndex};
      }
      ...
      <div className="attributegrid">
        ...
        <select name="Set 1" 
          ...
          {selection.map((item, index) => (
            <option disabled={(item.selected === index && item.state) ? true : false}>{item.value}</option>
          ))}
        </select>
        <Social />
        <select name="Set 2" 
          ...
          <option>-- selection an option --</option>
          {selection.map((item, index) => (
            <option disabled={(item.selected === index && item.state === true) ? true : false}>{item.value}</option>
          ))}
        </select>
        <Mental />
        <select name="Set 3" 
          ...
          <option>-- selection an option --</option>
          {selection.map((item, index) => (
            <option id={item.value} disabled={(item.selected === index && item.state === true) ? true : false}>{item.value}</option>
          ))}
        </select>
      </div>
    </>
